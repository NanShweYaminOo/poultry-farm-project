/* ==========================================================================
   Broiler Farming System — Admin Profile
   Loads the current admin's real profile from /api/v1/users/me and wires
   Change Photo / Cancel / Save Changes to the real backend (profile info,
   password change, photo upload) via AdminUI.authFetch.
   ========================================================================== */
...
(function () {
  ...
  function renderAvatar(profileImageUrl, fullName) {
    ...
    if (profileImageUrl) {
      ...
      avatar.textContent = fullName ? fullName.trim().slice(0, 2).toUpperCase() : 'AD';
    }
  }
  ...
  function loadProfile() {
    window.AdminUI.authFetch('/api/v1/users/me')
      .then(function (response) { return response.json(); })
      .then(renderProfile)
      .catch(function () {
        window.AdminUI.toast('ပရိုဖိုင်တင်ရန် မအောင်မြင်ပါ။', 'Failed to load profile.', 'bi-exclamation-triangle-fill');
      });
  }
  ...
  function initChangePhoto() {
    ...
    input.addEventListener('change', function () {
      ...
      window.AdminUI.authFetch('/api/v1/users/me/photo', { method: 'POST', body: formData })
        ...
        .then(function (result) {
          ...
          window.AdminUI.toast('ပရိုဖိုင်ပုံ ပြောင်းလဲပြီးပါပြီ။', 'Profile photo updated.');
        })
        .catch(function () {
          window.AdminUI.toast('ပုံတင်၍ မရပါ။', 'Could not upload photo.', 'bi-exclamation-triangle-fill');
        })
        .finally(function () {
          input.value = '';
        });
  ...
  function initSave() {
    ...
    form.addEventListener('submit', function (event) {
      ...
      if (!fullName || !phoneNumber) {
        window.AdminUI.toast('အမည်နှင့် ဖုန်းနံပါတ် ဖြည့်စွက်ပါ။', 'Full name and phone number are required.', 'bi-exclamation-triangle-fill');
        return;
      }
      if (wantsPasswordChange) {
        if (!currentPassword || !newPassword || !confirmPassword) {
          window.AdminUI.toast('စကားဝှက်ပြောင်းရန် ကွက်လပ်များအားလုံး ဖြည့်စွက်ပါ။', 'Fill in all password fields to change your password.', 'bi-exclamation-triangle-fill');
          return;
        }
        if (newPassword !== confirmPassword) {
          window.AdminUI.toast('စကားဝှက်အသစ် နှစ်ခု မတူညီပါ။', 'New password and confirmation do not match.', 'bi-exclamation-triangle-fill');
          return;
        }
        if (newPassword.length < 8) {
          window.AdminUI.toast('စကားဝှက်အသစ်သည် အနည်းဆုံး စာလုံး ၈ လုံး ရှိရမည်။', 'New password must be at least 8 characters.', 'bi-exclamation-triangle-fill');
          return;
        }
      }
      ...
      window.AdminUI.authFetch('/api/v1/users/me', {
        ...
        .then(function (result) {
          ...
          return window.AdminUI.authFetch('/api/v1/users/me/password', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        .then(function () {
          clearPasswordFields();
          window.AdminUI.toast('အပြောင်းအလဲများ သိမ်းဆည်းပြီးပါပြီ။', 'Changes saved.');
        })
        .catch(function (err) {
          window.AdminUI.toast(err.message, err.message, 'bi-exclamation-triangle-fill');
        })
        .finally(function () {
